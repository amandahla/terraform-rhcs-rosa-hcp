...
# Usage: install-release-tool.sh <addlicense|tflint|terraform-docs|vale|trivy> <version> <dest_dir>

set -euo pipefail

tool="${1:?tool name required (addlicense, tflint, terraform-docs, vale, or trivy)}"
...
case "$tool" in
  ...
  trivy)
    case "${os}_${arch}" in
      linux_amd64) platform_label="Linux-64bit" archive_ext="tar.gz" ;;
      linux_arm64) platform_label="Linux-ARM64" archive_ext="tar.gz" ;;
      darwin_amd64) platform_label="macOS-64bit" archive_ext="tar.gz" ;;
      darwin_arm64) platform_label="macOS-ARM64" archive_ext="tar.gz" ;;
      windows_amd64) platform_label="windows-64bit" archive_ext="zip" ;;
      *)
        echo "Unsupported platform for trivy: ${os}_${arch}" >&2
        exit 1
        ;;
    esac
    asset="trivy_${version_no_v}_${platform_label}.${archive_ext}"
    url="https://github.com/aquasecurity/trivy/releases/download/${version}/${asset}"
    checksums_url="https://github.com/aquasecurity/trivy/releases/download/${version}/trivy_${version_no_v}_checksums.txt"
    dest_bin="${dest_dir}/trivy"
    ...
    install -m 0755 "${tmp}/trivy" "$dest_bin"
    ;;
