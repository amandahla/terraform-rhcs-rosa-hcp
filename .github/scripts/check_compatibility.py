...
def llm_intro_version(resource_type: str, snippet: str) -> IntroResult:
    """Call OpenAI or Gemini to infer introduction version from changelog excerpt."""
    system = (
    ...
    if os.environ.get("OPENAI_API_KEY"):
        return _openai_json(system, user, resource_type)
    ...
    return IntroResult(
        ...
        confidence="low",
        evidence_line="",
        error="No OPENAI_API_KEY or GOOGLE_API_KEY set",
    )


def _openai_json(system: str, user: str, resource_type: str) -> IntroResult:
    try:
        from openai import OpenAI
    except ImportError:
        return IntroResult(
            resource_type, None, "low", "", "openai package not installed"
        )
    client = OpenAI()
...
def main() -> int:
    ...
    if not os.environ.get("OPENAI_API_KEY") and not os.environ.get("GOOGLE_API_KEY"):
        print("No OPENAI_API_KEY or GOOGLE_API_KEY: skipping compatibility check (no PR comment).")
        return 0
